"""Build apex_audit_trail_export_20260512.xlsx (Apex class coverage workbook).

Same layout as build_mainclass_testclass_coverage_workbook.py but reads the
20260512 test-run results and writes to the user's Downloads folder.
"""
...
RUN_DIR = ROOT / "tmp" / "coverage_all_20260512"
CLASSES_DIR = ROOT / "force-app" / "main" / "default" / "classes"

DOWNLOADS = Path(os.path.expanduser("~")) / "Downloads"
OUT_PATH = DOWNLOADS / "apex_audit_trail_export_20260512.xlsx"


def find_result_json(run_dir):
    candidates = [
        p for p in run_dir.glob("test-result-*.json")
        if "codecoverage" not in p.name and "junit" not in p.name
    ]
    if not candidates:
        raise FileNotFoundError(f"No test-result-*.json under {run_dir}")
    # prefer the one ending with the run id; otherwise newest
    candidates.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    return candidates[0]


def find_codecov_json(run_dir, result_json):
    # Prefer the aggregate per-class file (test-result-codecoverage.json) over
    # the per-test-method file (test-result-{runid}-codecoverage.json), since
    # the aggregate has totalLines/totalCovered/coveredPercent per class.
    alt = run_dir / "test-result-codecoverage.json"
    if alt.exists():
        return alt
    stem = result_json.stem
    paired = run_dir / f"{stem}-codecoverage.json"
    if paired.exists():
        return paired
    raise FileNotFoundError("No codecoverage json found")
...
def main():
    ...
    codecov_json = find_codecov_json(RUN_DIR, result_json)
    print(f"[in] result={result_json.name}")
    print(f"[in] coverage={codecov_json.name}")

    result = load_json(result_json)
    result = result.get("result", result)
    summary = result["summary"]
    coverage_records = result.get("coverage", {}).get("records", [])
    class_coverage = load_json(codecov_json)
    if isinstance(class_coverage, dict) and "result" in class_coverage:
        class_coverage = class_coverage["result"]
